...
const COLUMNS = [
  ...
  { key: 'policy', label: 'Policy & legal research' },
  { key: 'fiscal', label: 'Fiscal / budget research' },
  { key: 'grassroots', label: 'Grassroots & community' },
  { key: 'civic', label: 'Civic & rights advocacy' },
  { key: 'party', label: 'Party-affiliated' },
  { key: 'student', label: 'Student organizing' },
  { key: 'dcadvocacy', label: 'DC Community Advocacy' },
  { key: 'natladvocacy', label: 'National Advocacy' },
...
const DATA = {
  inner: [
    {
      ...
      type: 'Official government body', scope: 'local', url: 'https://statehood.dc.gov/',
    ...
    },
    {
      ...
      type: 'Policy research & advocacy', scope: 'local', url: 'https://www.dcvote.org/',
      blurb: 'DC Vote',
    ...
    {
      id: 'neighbors-united', name: 'Neighbors United for DC Statehood', col: 'grassroots',
      type: 'Grassroots organizing', scope: 'local', url: 'https://www.the51st.org/',
    ...
    },
    {
      ...
      type: 'Student / youth organizing', scope: 'national', url: 'http://studentsfordcstatehood.com/',
      blurb: 'Students for Statehood',
    ...
    },
    {
      ...
      type: 'Party-affiliated organizing', scope: 'local', url: 'https://www.dcdemocraticparty.org/dcstatehood',
    ...
    {
      id: 'statehood-questionnaire', name: 'Statehood Candidate Questionnaire', col: 'dcadvocacy',
      type: "RepresentDC's own tool", scope: 'local', url: 'https://candidates.representdc.org',
      blurb: 'Candidate Questionnaire',
      desc: "RepresentDC's own tool, tracking where 2026 D.C. candidates stand on statehood commitments — D.C.-focused, distinct from the national Statehood Pledge below despite the similar name.",
    },
    {
      id: 'statehood-pledge', name: 'Statehood Pledge', col: 'natladvocacy',
      type: 'Independent volunteer-run tool', scope: 'national', url: 'https://www.dcstatehoodpledge.org/',
    ...
    {
      id: 'statehood-compact', name: 'Statehood Compact', col: 'natladvocacy',
      type: 'Independent volunteer-run tool', scope: 'national', url: 'https://www.dcstatehoodcompact.org/',
    ...
    {
      id: 'statehood-scorecard', name: 'Statehood Scorecard', col: 'natladvocacy',
      type: 'Independent volunteer-run tool', scope: 'national', url: 'https://www.dcstatehoodscorecard.org/',
    ...
    },
    {
      ...
      type: 'Funding / PAC', scope: 'national', url: 'https://www.dcstatehoodpac.com/',
      blurb: 'Statehood PAC',
    ...
    },
    {
      ...
      type: 'Coalition / partnership', scope: 'national', url: 'https://togetherfordc.org/',
  ...
  middle: [
    {
      id: 'lwv-dc', name: 'League of Women Voters of DC', col: 'civic',
      type: 'Nonpartisan civic engagement', scope: 'local', url: 'https://www.lwvdc.org/',
      blurb: 'LWV DC',
    ...
    {
      id: 'aclu-dc', name: 'ACLU of DC', col: 'civic',
      type: 'Civil liberties advocacy', scope: 'local', url: 'https://www.acludc.org/',
      blurb: 'ACLU DC',
    ...
    {
      id: 'free-dc', name: 'Free DC', col: 'grassroots',
      type: 'Grassroots / home-rule defense', scope: 'local', url: 'https://freedcproject.org/',
      blurb: 'Free DC',
  ...
  outer: [
    {
      ...
      type: 'Public-interest law & governance', scope: 'local', url: 'https://www.dcappleseed.org/',
      blurb: 'DC Appleseed',
    ...
    {
      id: 'dcfpi', name: 'DC Fiscal Policy Institute', col: 'fiscal',
      type: 'Budget / tax policy research', scope: 'local', url: 'https://www.dcfpi.org/',
      blurb: 'DCFPI',
    ...
    {
      id: 'anacostia-cc', name: 'Anacostia Coordinating Council', col: 'grassroots',
      type: 'Community / neighborhood association', scope: 'local', url: 'https://www.anacostiacc.org/',
    ...
    {
      id: 'naacp-dc', name: 'NAACP (DC Branch)', col: 'civic',
      type: 'Civil rights advocacy', scope: 'national', url: 'https://naacp.org/',
      blurb: 'NAACP (DC)',
...
function buildGrid() {
  ...
  ROWS.forEach((r) => {
    grid[r.key] = {}
    COLUMNS.forEach((c) => { grid[r.key][c.key] = [] })
    DATA[r.key].forEach((org) => { grid[r.key][org.col].push(org) })
  })
  return grid
}
    ...
    <div className="app">
      ...
      <section className="impact-section spm-matrix-section">
        <div className="container">
              ...
              <table className="spm-matrix">
                <thead>
                  <tr>
                    ...
                    {COLUMNS.map((c) => (
                      <th key={c.key}><span className="spm-col-label">{c.label}</span></th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {ROWS.map((r) => (
                    <tr key={r.key} className={`spm-row-${r.key}`}>
                      <th>
                        <span className="spm-row-label">{r.label}</span>
                        <span className="spm-row-sub">{r.sub}</span>
                      </th>
                      {COLUMNS.map((c) => (
                        <td key={c.key} className="spm-cell">
                          {grid[r.key][c.key].length > 0 && (
                            <div className="spm-chip-stack">
                              {grid[r.key][c.key].map((org) => (
                                <a
                                  key={org.id}
                                  href={`#${org.id}`}
                                  className={`spm-chip spm-row-${r.key} spm-scope-${org.scope}`}
                                >
                                  {org.blurb}
                                </a>
                              ))}
          ...
          <p className="spm-matrix-note">
            Empty cells are informative, not missing data — e.g. no Outer-layer org happens
            to be Official-government-type or Party-affiliated. Click any chip to jump to its
            full entry below. Scroll right for more columns; row labels stay pinned.
          </p>
        </div>
      </section>
      ...
      <section className="impact-section">
        <div className="container">
          ...
          {ROWS.map((r) => (
            <div className="spm-layer-block" key={r.key}>
              <h3 className={`spm-layer-heading spm-row-${r.key}`}>
                <span className="spm-layer-dot"></span>{r.label} layer
              </h3>
              <p className="spm-layer-desc">{r.desc}</p>
                ...
                {DATA[r.key].map((org) => (
                  <div className="spm-org-card" id={org.id} key={org.id}>
                    ...
                    <div className="spm-org-tags">
                      <span className="spm-tag">{org.type}</span>
